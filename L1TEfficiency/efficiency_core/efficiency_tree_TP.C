...
CutReader probeCuts(ProbeCuts);
 ...
 float pt_tag,eta_tag,phi_tag,minDRtagl1,isotag,pt_l1tag,eta_l1tag,phi_l1tag,DRtagl1;
 ...
 t1->Branch("pt_tag",& pt_tag); t1->Branch("eta_tag",& eta_tag);
 t1->Branch("phi_tag",& phi_tag); t1->Branch("iso_tag",& isotag);
 t1->Branch("pt_l1tag",& pt_l1tag); t1->Branch("eta_l1tag",& eta_l1tag);
 t1->Branch("phi_l1tag",& phi_l1tag); t1->Branch("DR_l1tag",& DRtagl1);
 ...
 t1->Branch("DR_l1probe",& DRprobel1); t1->Branch("DR_tag_probe",& DRtagprobe);
...
cout<<"total "<<cc_reco->GetEntries()<<endl;
int evt=0;
while(recoReader.Next() && gmtReader.Next()){
 ...
 if (TF=="bmtf" || TF=="omtf" || TF=="emtf") 
   tfReader.Next();
 evt++;
// cout<<"nreco "<<*numreco<<endl;
 if (*numreco<2) continue;
 pt_tag=-9,eta_tag=-9,phi_tag=-9,minDRtagl1=-9,isotag=-9,pt_l1tag=-9,eta_l1tag=-9,phi_l1tag=-9,DRtagl1=-9;
pt_probe.clear(); eta_probe.clear(); phi_probe.clear(); DRprobel1.clear();
isoprobe.clear(); pt_l1probe.clear(); eta_l1probe.clear(); phi_l1probe.clear(); 
DRtagprobe.clear(); qual_l1probe.clear();
...
for (unsigned int j=0; j<*numreco; j++){
 ...
 for (unsigned int it=0; it<tagindex.size(); it++){
    ...
    if (mupairs[0].second>tagCuts.drL1RecoCone()) continue;
    l1tagpair.emplace_back(make_pair(mupairs[0].first,itag));
 }
 
 if (l1tagpair.size()==0) continue;
 for(auto it: l1tagpair){
   unsigned int itag=it.second,il1=it.first;
   pt_tag=ptreco[itag]; eta_tag=etareco[itag]; phi_tag=phireco[itag]; 
   isotag=isoreco[itag]; pt_l1tag=l1mupt[il1]; eta_l1tag=l1mueta[il1]; 
   phi_l1tag=l1muphi[il1];
   DRtagl1=matchDR(etareco[itag],phireco[itag],l1mueta[il1],l1muphi[il1]);
   TLorentzVector muTag; muTag.SetPtEtaPhiM(ptreco[itag],etareco[itag],phireco[itag],0.105);
   for (unsigned int ip=0; ip<probeindex.size(); ip++){
      ...
      TLorentzVector muProbe; muProbe.SetPtEtaPhiM(ptreco[iprobe],etareco[iprobe],phireco[iprobe],0.105);
      if ((muTag+muProbe).M()<probeCuts.minMll() || (muTag+muProbe).M()>probeCuts.maxMll()) continue;
      if ( (TF=="bmtf" || TF=="omtf" || TF=="emtf") && matchDR(muProbe.Eta(),muProbe.Phi(),muTag.Eta(),muTag.Phi())<(tagCuts.drL1RecoCone()*2))
         continue;
      pt_probe.push_back(ptreco[iprobe]); eta_probe.push_back(etareco[iprobe]); 
      phi_probe.push_back(phireco[iprobe]); isoprobe.push_back(isoreco[iprobe]);
      ...
      if(mupairs.size()>0) {
        ...
        if (mupairs[0].second>probeCuts.drL1RecoCone()) continue;
