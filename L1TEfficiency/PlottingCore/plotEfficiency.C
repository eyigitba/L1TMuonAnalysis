...
TLatex head(){
  ...
  header.DrawLatexNDC(0.63, 0.92, "#sqrt{s} = 13 TeV, UL16MC ");
  return header; 
}
...
int plotEfficiency( std::string plots, TString ntuple, TString path, TString ntupleEmul="none"){
 ...
 while (std::getline(infile, line)){
   ...
   TH1F *temp_den,*temp_num,*temp_denEmul,*temp_numEmul;


   //create efficiencies
   temp_den=gethisto(cc,tokens[1],tokens[2],"hden_"+tokens[0],std::stoi(tokens[5]),std::stof(tokens[6]),std::stof(tokens[7]));
   temp_num=gethisto(cc,tokens[1],tokens[2]+" && "+tokens[3],tokens[0],std::stoi(tokens[5]),std::stof(tokens[6]),std::stof(tokens[7]));
   ...
   error->GetYaxis()->SetRangeUser(0.00001,1.2);
   ...
   if (temp_leg.size()>0) 
      ...
      legs.push_back("DY PYTHIA8"); 

   kwds.push_back(options);
   errors.push_back(error);
   if (ntupleEmul!="none"){
     canvasname.push_back(tokens[8]); 
     legs.push_back("DY POWHEG");
     kwds.push_back(options);
