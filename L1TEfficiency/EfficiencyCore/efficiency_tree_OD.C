...
TChain*cc_muon=new TChain(L1tree);
...
std::vector<float> pt_probe,eta_probe,phi_probe,DRprobel1,isoprobe,
                   pt_l1probe,eta_l1probe,phi_l1probe,DRtagprobe,qual_l1probe;
...
while(recoReader.Next() && gmtReader.Next()){
 // if we want to read TF output
 if (TF=="bmtf" || TF=="omtf" || TF=="emtf")
     tfReader.Next();
 // clear stuff
 isoprobe.clear(); pt_probe.clear(); eta_probe.clear(); phi_probe.clear();
 DRprobel1.clear(); pt_l1probe.clear(); eta_l1probe.clear();
 phi_l1probe.clear(); qual_l1probe.clear();

 // find good reco muons
 for (int j=0; j<*numreco; j++){
  ...
  isoprobe.push_back(isoreco[j]);
  pt_probe.push_back(ptreco[j]);
  eta_probe.push_back(etareco[j]);
  phi_probe.push_back(phireco[j]);
 ...
 if(mupairs.size()>0){
   ...
   if (mupairs[0].second>recoCuts.maxdrL1RecoCone()) continue;
